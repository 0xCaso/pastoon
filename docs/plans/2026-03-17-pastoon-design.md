...
│   ├── cli.ts           -- CLI entry point (incur-based)
...
| `incur` | CLI framework with built-in MCP server (`--mcp`), skill file generation (`skills add`), agent registration (`mcp add`), TOON output, Zod schemas, shell completions |
...
# Setup: installs LaunchAgent + starts menu bar tray (clipboard auto-conversion)
pastoon setup
...
# MCP server (incur built-in --mcp flag)
pastoon --mcp

# Register pastoon as MCP server with all detected coding agents
# (incur built-in: detects Claude Code, OpenCode, Cursor, Windsurf, etc.)
pastoon mcp add

# Generate/sync SKILL.md for coding agents
# (incur built-in: outputs to ./SKILL.md or agent config dir)
pastoon skills add
```
...
`pastoon --mcp` starts an MCP server over stdio transport (via incur's
built-in `--mcp` flag — no custom MCP setup code required).
...
> **Note:** No `clipboard_to_toon` MCP tool. Agents work with strings in
> context, not the OS clipboard — such a tool would be useless to them.

### MCP registration

`pastoon mcp add` (incur built-in) detects installed coding agents by
checking known config file paths and registers the server automatically.

Example of what it writes for each agent:
...
Generated via `pastoon skills add` (incur built-in). Can be customized after
generation and re-synced. Placed in `AGENTS.md`, `CLAUDE.md`, or whatever
system prompt file the user's agent reads.
